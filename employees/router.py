...
from employees.schemas import CreateEmployeePayload, GlobalEmployeeResponse, ListEmployeeResponse
...
@router.post("", status_code=status.HTTP_201_CREATED, response_model=GlobalEmployeeResponse)
async def create_employee(body: CreateEmployeePayload, db: AsyncSession = Depends(get_db)):
...
async def search_employee(name: str = "", db: AsyncSession = Depends(get_db)):
    return await service.search_employee(db, name)
