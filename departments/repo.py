...
async def update_by_id(db: AsyncSession, id, data: dict) -> Department:
    ...
    try: 
        ...
        raise ConflictException(detail=f"{data.get("name") or "department"} already exist")
    except NoResultFound:
        raise NotFoundException(detail=f"department not found")
...
async def insert_employee_to_department(db: AsyncSession, id: int, employee_id: int) -> Employee_X_Department:
    ...
    try:
        ...
        await db.rollback()
        raise ConflictException(detail=f"employee already exist in the department")
...
async def delete_employee_from_department(db: AsyncSession, id: int, employee_id: int) -> Employee_X_Department:
    ...
    try:
        ...
        raise NotFoundException(detail=f"employee to department record not found")
